...
VALID_PROFILES = [
    os.path.join(VALID_PROFILE_DIR, f) for f in os.listdir(VALID_PROFILE_DIR)
]


@pytest.mark.parametrize("path, expected_exc", [(VALID_PROFILES[0], None)])
def test_fhir_format(caplog, tmpdir, path, expected_exc):
    format_dict = {".xml": "json", ".json": "xml"}
    ...
    output_str, out_file = loader.fhir_format(
        path,
    ...
    output_str, _ = loader.fhir_format(
        out_file,
    ...
    [
        (
            [
                ...
                os.path.join(EXTENSION_DIR, "valid"),
            ],
            0,
        ),
        (
            [
                ...
                os.path.join(EXAMPLE_DIR, "invalid"),
            ],
            1,
        ),
    ],
...
def test_ig_validation(temp_site_root, dir_list, expected_code):
    """
    Test ncpi_fhir_utility.app.validate
    """
    runner = CliRunner()
...
@pytest.mark.parametrize("resource_dict, error_msg", INVALID_RESOURCES)
def test_custom_validate(resource_dict, error_msg):
    """
    Test ncpi_fhir_utility.app._custom_validate
    """
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            os.path.join(EXAMPLE_DIR, "invalid", "Patient-pt-001.json"),
...
def test_update_ig(debug_caplog, filepath, error_msg):
    ...
    result = runner.invoke(
        cli.add,
        [filepath, "--ig_control_file", os.path.join(TEST_SITE_ROOT, "ig.ini")],
    )
    assert result.exit_code != 0
...
def test_remove_old_ig_state(temp_site_root):
    ...
    filepaths = copy_resources_into_ig([profiles_dir, ext_dir], temp_site_root)
    ...
    ig_filenames = resource_files_in_ig(ig_control_filepath)
    extension_filenames = {fn for fn in os.listdir(ext_dir)}
